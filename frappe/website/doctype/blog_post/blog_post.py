...
class BlogPost(WebsiteGenerator):
	...
	@frappe.whitelist()
	def set_my_blogger(self):
		if not self.blogger:
			bloggers = frappe.db.sql("""SELECT `name` FROM `tabBlogger` WHERE `user` = '{user}'""".format(user=frappe.session.user), as_dict=True)
			if len(bloggers) > 0:
				return bloggers[0].name
		return self.blogger
